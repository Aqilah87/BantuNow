...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  // ── Toggle availability — update users + semua open post bantuan ──
  Future<void> _toggleAvailability(bool isMalay) async {
    ...
    try {
      ...
      // 2️⃣ Batch update SEMUA open post bantuan milik user
      //    — supaya filter Best Match "requireAvailable" berfungsi
      //    — supaya warning banner dalam detail screen berfungsi
      ...
      if (posts.docs.isNotEmpty) {
        ...
        for (final doc in posts.docs) {
          batch.update(doc.reference, {'poster_availability': newStatus});
        }
        await batch.commit();
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(newStatus == 'available'
              ? (isMalay
                  ? '✅ Anda kini Available! Post anda aktif dalam Best Match.'
                  : '✅ You are now Available! Posts active in Best Match.')
              : (isMalay
                  ? '⚫ Tidak Available. Post masih kelihatan tapi ditandakan.'
                  : '⚫ Unavailable. Posts still visible but marked.')),
          backgroundColor:
              newStatus == 'available' ? Colors.green : Colors.grey.shade700,
                                  ...
                                  child: Row(children: [
                                    ...
                                    Expanded(
                                      child: Text(
                                        _isAvailable
                                            ? (isMalay
                                                ? 'Post anda aktif dalam filter Best Match. Orang boleh jumpa anda lebih mudah.'
                                                : 'Your posts are active in Best Match filter. People can find you easier.')
                                            : (isMalay
                                                ? 'Post anda masih kelihatan tapi ditandakan "Tidak Available". Orang masih boleh hubungi anda.'
                                                : 'Your posts are still visible but marked "Unavailable". People can still contact you.'),
                                        style: TextStyle(
                                            fontSize: 11,
                                            color: _isAvailable
